...
void FeatureTracker::trackFeatures(
  ...
  cv::drawKeypoints(img_2, keypoints_2, img_keypoints_2);

  // Save images
  cv::imwrite("keypoints_1.png", img_keypoints_1);
  cv::imwrite("keypoints_2.png", img_keypoints_2);

  // Show images
  // cv::imshow("keypoints_1", img_keypoints_1);
  // cv::imshow("keypoints_2", img_keypoints_2);
  ...
  // Draw matches
  // drawMatches(img_1, img_2, keypoints_1, keypoints_2, matches);
  ...
  // Create vectors with matched keypoints
  std::vector<cv::KeyPoint> matched_kp_1, matched_kp_2;
  ...
  // Calculate number of inliers by counting 1s in the inlier mask
  ...
  drawCombinedImage(img_1, img_2, keypoints_1, keypoints_2, good_matches,
                    inlier_mask);
  ...
  avg_num_keypoints_img1_ = (avg_num_keypoints_img1_ * old_num_samples +
                             static_cast<float>(keypoints_1.size())) /
                            new_num_samples;
  avg_num_keypoints_img2_ = (avg_num_keypoints_img2_ * old_num_samples +
                             static_cast<float>(keypoints_2.size())) /
                            new_num_samples;
  avg_num_matches_ = (avg_num_matches_ * old_num_samples +
                      static_cast<float>(matches.size())) /
                     new_num_samples;
  avg_num_good_matches_ = (avg_num_good_matches_ * old_num_samples +
                           static_cast<float>(good_matches.size())) /
                          new_num_samples;
  avg_num_inliers_ =
      (avg_num_inliers_ * old_num_samples + static_cast<float>(num_inliers)) /
      new_num_samples;
  avg_inlier_ratio_ = (avg_inlier_ratio_ * old_num_samples +
 ...
 */
void FeatureTracker::drawCombinedImage(
    const cv::Mat& img_1,
    const cv::Mat& img_2,
...
void FeatureTracker::printStats() const {
  std::cout << "Avg. Keypoints 1 Size: " << avg_num_keypoints_img1_
            << std::endl;
  std::cout << "Avg. Keypoints 2 Size: " << avg_num_keypoints_img2_
            << std::endl;
  std::cout << "Avg. Number of matches: " << avg_num_matches_ << std::endl;
  std::cout << "Avg. Number of good matches: " << avg_num_good_matches_
            << std::endl;
