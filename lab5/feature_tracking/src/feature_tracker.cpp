    ...
    std::pair<std::vector<cv::KeyPoint>, std::vector<cv::KeyPoint>>* matched_kp_1_kp_2)
{
    ...
    // Detect keypoints
    tracker.detectKeypoints(img_1, &keypoints_1);
    tracker.detectKeypoints(img_2, &keypoints_2);
    ...
    avg_inlier_ratio_ = (avg_inlier_ratio_ * old_num_samples + (static_cast<float>(num_inliers) / static_cast<float>(good_matches.size()))) / new_num_samples;
    ++num_samples_;
